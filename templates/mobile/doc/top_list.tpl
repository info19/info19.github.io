...
$(document).ready(function(){
	Xajax = new AjaxUpload('upload_2', {
		...
		onSubmit: function (file, ext) {
			if(!(ext && /^(doc|docx|xls|xlsx|ppt|pptx|rtf|pdf|png|jpg|gif|psd|mp3|djvu|fb2|ps|jpeg|txt|rar|zip|dmg)$/.test(ext))) {
		...
		onComplete: function (file, row){
			if(row == 1)
				addAllErr('Превышен максимальный размер файла 1000 МБ', 3300);
			else {
				row = row.split('"');
...
У Вас <span id="upDocNum">{doc-num}</span> <span id="langNumric"></span>
</div>
<div class="cover_edit_title fixed">
<div class="upload_doc"><button id="upload_2">Загрузить документ</button></div>
<div class="clear"></div>
