...
<div class="wall_signed"><a class="wall_signed_by" href="/u{auth_id}"><div  style="float: left;" class="txt_pod" id="p{user-id}" onmouseover="pagetooltip((this), '{ava_two}', '{author_view}', '/u{auth_id}', '{user_status}', '(this.id)');">{author_view}</div></a></div>
<div class="size10 infowalltext_f clear">
  <div class="fl_l"><a href="/wallgroups{public-id}_{rec-id}" onClick="Page.Go(this.href); return false" class="online">{date}</a> [privacy-comment][comments-link]<span id="fast_comm_link_{rec-id}" class="fast_comm_link">&nbsp;|&nbsp; <a href="/" id="fast_link_{rec-id}" onClick="wall.open_fast_form('{rec-id}'); wall.fast_open_textarea('{rec-id}'); return false">Комментировать</a></span>[/comments-link][/privacy-comment]</div>
  ...
  <div class="fl_r public_wall_like cursor_pointer" onClick="{like-js-function}" onMouseOver="groups.wall_like_users_five('{rec-id}')" onMouseOut="groups.wall_like_users_five_hide('{rec-id}')" id="wall_like_link{rec-id}">
   ...
   <div class="wall_tell_all cursor_pointer" onMouseOver="myhtml.title('{rec-id}', 'Отправить в сообщество или другу', 'wall_tell_all_')" onClick="Repost.Box('{rec-id}', 1); return false "id="wall_tell_all_{rec-id}" style="margin-top:-5px;margin-right:2px;[owner]margin-top:-5px;margin-right:2px[/owner]"></div>
  [privacy-comment][comments-link]<div class="wall_fast_form no_display" id="fast_form_{rec-id}" style="width: 100%; margin-top: 29px; box-shadow: none; padding-bottom: 1px; margin-left: -14px; border-top-width: 1px; border-top-style: solid; border-top-color: rgb(218, 225, 232);">
...
<div class="clear"></div>
[/record]
[all-comm]<div class="cursor_pointer" onClick="groups.wall_all_comments('{rec-id}', '{public-id}'); return false" id="wall_all_but_link_{rec-id}"><div class="public_wall_all_comm" id="wall_all_comm_but_{rec-id}" style="    border: 7px solid #fff;
    box-shadow: 0 1px 0 0 #d7d8db, 0 0 0 1px #e3e4e8;">Показать {gram-record-all-comm}</div></div>[/all-comm]
...
[comment-form]<div class="wall_fast_opened_form" id="fast_form">
 <input type="text" class="wall_inpst fast_form_width wall_fast_input" value="Комментировать..." id="fast_inpt_{rec-id}" onMouseDown="wall.fast_open_textarea('{rec-id}', 2); return false" style="    width: 430px;    border-radius: 2px;    height: 20px;margin:0px" />
 <div class="no_display wall_fast_texatrea" id="fast_textarea_{rec-id}">
