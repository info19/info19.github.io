	...
	Xajax = new AjaxUpload('upload_2', {
		...
		onComplete: function (file, row){
			if(row == 1)
				...
				row = row.split('"');
				$('#loadedDocAjax').html('<div class="doc_block" style="margin-left:0px;margin-right:0px" id="doc_block'+row[1]+'"><a href="/index.php?go=doc&act=download&did='+row[1]+'"><div class="doc_format_bg cursor_pointer"><img src="{theme}/images/darr.gif" style="margin-right:5px" />'+row[3]+'</div></a><div id="data_doc'+row[1]+'"><a href="/index.php?go=doc&act=download&did='+row[1]+'"><div class="doc_name cursor_pointer" id="edit_doc_name'+row[1]+'" style="max-width:580px">'+row[0]+'</div></a>
<img class="fl_l cursor_pointer" style="margin-top:5px;margin-left:5px" src="{theme}/images/close_a.png" onClick="Doc.Del('+row[1]+')" onMouseOver="myhtml.title('+row[1]+', \'Удалить документ\', \'wall_doc_\')" id="wall_doc_'+row[1]+'" /></div>
<div id="edit_doc_tab'+row[1]+'" class="no_display">
<input type="text" class="inpst doc_input" value="'+row[0]+'" maxlength="60" id="edit_val'+row[1]+'" size="60" />
<div class="clear" style="margin-top:5px;margin-bottom:35px;margin-left:62px"><div class="button_div fl_l">
<button onClick="Doc.SaveEdit('+row[1]+', \'editLnkDoc'+row[1]+'\')">Сохранить</button></div>
<div class="button_div_gray fl_l margin_left">
<button onClick="Doc.CloseEdit('+row[1]+', \'editLnkDoc'+row[1]+'\')">Отмена</button></div></div> </div>
<div class="doc_sel" onClick="Doc.ShowEdit('+row[1]+', this.id)" id="editLnkDoc'+row[1]+'">Редактировать</div>
<div class="doc_date clear">'+row[2]+', Добавлено '+row[4]+'</div><div class="clear"></div></div>'+$('#loadedDocAjax').html());
...
<div class="cover_edit_title fixed" style="width:594px">
