  ...
  create: function(){
	...
	if(name != 0){
	  ...
	  $.post('/index.php?go=apps_a&act=create', {name: name}, function(d){
	   ...
	   if(json.app_id){
	    butloading('createNew', 83, 'enabled', 'Создать');
		    ...
		    if(key){
						    if(url){
	                           ...
	                           $.post('/index.php?go=apps_a&act=save', {name: name, key: key, meth: meth, url: url, id: id, mname1: mname1, mname2: mname2, mname3: mname3, cur: cur, callbuy: callbuy}, function(d){
	                              if(d){
	                                 butloading('save', 83, 'enabled', 'Сохранить');
	                              }
	                            });
...
//MESSAGES
var messages = {
    pnew_: function(user_id){
                var content = '<div style="padding:20px">'+
                '<div class="texta" style="width:100px">Кому:</div><input type="text" value="" id="theme" class="inpst" maxlength="255" style="width:300px;padding: 10px 4px 5px 4px;"><nobr1>Администратору</nobr1><div class="mgclr"></div>'+
                '<div class="texta" style="width:100px">Сообщение:</div><textarea id="msg" class="inpst" style="width:292px;height:120px;"></textarea><div class="mgclr"></div>'+
                '</div>';
                Box.Show('new_msg', 470, lang_new_msg, content, lang_box_canсel, lang_new_msg_send, 'messages.send('+user_id+'); return false');
                $('#msg').focus();
    },	
	new_: function(user_id){
		var content = '<div style="padding:20px">'+
		'<textarea id="msg" class="inpst" style="width:408px;height:120px;" placeholder="Сообщение:"></textarea><div class="mgclr"></div>'+
		'</div>';
...
                Box.Page('/index.php?go=graffiti&id='+id+'&act=add', '', 'all_graffiti', 600, 'Граффити на стену', lang_box_canсel, 0, 0, 0, 1, 0, 1, 0, 0);
        },
...
'<div class="photo_close" onClick="Photo.Close(\'\'); return false;"></div>'+
 '<div class="photo_bg" style="min-height:400px">'+
  '<div class="photo_com_title" style="padding-top:0px;padding: 11px;font-size: 14px;">'+topTxt+'<div><a href="/" onClick="Photo.Close(\'\'); return false"></a></div></div>'+
...
var im = {
	...
	box_del: function(u){
		Box.Show('im_del'+u, 450, 'Подтверждение действия', '<div style="padding:20px;line-height:150%;" id="del_status_text_im">  Вы действительно хотите удалить историю переписки? <b</b><br /><br><b> Примечание:</b> история переписки удалится только для вас, для вашего собеседника история удалена не будет!</div>', lang_box_canсel, lang_box_yes, 'im.del('+u+')');
	},
	del: function(u){
		$('#box_loading').show();
		ge('box_butt_create').disabled = true;
