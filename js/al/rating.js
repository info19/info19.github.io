var rating = {
 ...
 checkVote: function(n){
if(n == 1){
   $("#votenum").load('/index.php?go=rating&act=plus3');
} else if(n == 2){
   $("#votenum").load('/index.php?go=rating&act=plus8');
} else if(n == 3){
   $("#votenum").load('/index.php?go=rating&act=plusover');
} else if(n == 0){
   $("#votenum").load('/index.php?go=rating&act=plus1');
}
  },
...
view_menu: function(rid){
		$('#r_'+rid).hide('slow');
		$.post('/index.php?go=rating&act=view_menu', {rid: rid});
},
  ...
  stats: function(){
    viiBox.start();
	$.post('/index.php?go=rating&act=stats', function(d){
	  viiBox.win('stats', d);
	});
  },
  page: function(){
