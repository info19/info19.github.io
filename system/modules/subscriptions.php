...
//if($logged){
	...
	switch($act){
		...
		case "add":
			...
			if(!$CheckBlackList AND !$check AND $for_user_id != $user_id){
				...
				if($row_owner['user_last_visit'] >= $update_time){
					...
					$myRow = $db->super_query("SELECT user_sex, alias FROM `".PREFIX."_users` WHERE user_id = '{$user_info['user_id']}'");
					...
					if ($myRow['alias'] != null && $myRow['alias'] != 'empty') {
						$link = '/' . $myRow['alias'];
					} else {
		...
		default:
			...
			$sql_ = $db->super_query("SELECT tb1.friend_id, tb2.user_search_pref, user_photo, user_country_city_name, user_status, alias FROM `".PREFIX."_friends` tb1, `".PREFIX."_users` tb2 WHERE tb1.user_id = '{$for_user_id}' AND tb1.friend_id = tb2.user_id AND tb1.subscriptions = 1 ORDER by `friends_date` DESC LIMIT {$limit_page}, {$gcount}", 1);
			...
			if($sql_){
				...
				foreach($sql_ as $row){
					...
					if ($row['alias'] != null && $row['alias'] != 'empty') {
						$link = '/' . $row['alias'];
					} else {
