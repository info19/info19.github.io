	...
	switch($act){
		...
		default:
			...
			if($owner['user_public_num']){
				...
				$tpl->set('[no]', '');
				$tpl->set('[/no]', '');
				$tpl->set('{rec}', $tpl->result['recommendations_groups']);
				$tpl->set_block("'\\[yes\\](.*?)\\[/yes\\]'si","");
			}
			$tpl->compile('info');
			
	if(!$owner['user_public_num']){				
			$rec_ = $db->super_query("SELECT id, title, photo, traf, adres, ulist FROM `".PREFIX."_communities` WHERE del = '0' AND ban = '0' AND photo != '' ORDER by rand() LIMIT 6", 1);
			
	if($rec_){		
	

			$tpl->load_template('recommendations/groups.tpl');
			foreach($rec_ as $row1){
				
				$tpl->set('{ava}', '/uploads/groups/'.$row1['id'].'/100_'.$row1['photo']);
				$tpl->set('{public-id}', $row1['id']);
				$tpl->set('{name}', stripslashes($row1['title']));
				if(stripos($row1['ulist'], "|{$user_id}|") !== false){
					$tpl->set('[yes-group]', '');
					$tpl->set('[/yes-group]', '');
				...
				$tpl->set('{note-id}', $row1['id']);
				$tpl->set('{traf}', $row1['traf'].' '.gram_record($row1['traf'], 'groups_users'));
				if($row1['adres']) $tpl->set('{adres}', $row1['adres']);
				else $tpl->set('{adres}', 'public'.$row1['id']);
				$tpl->compile('info');
			}
