...
$modified = strtr($variables, array(
        ':)'=>'<img src=\'http://studentosi.ru/uploads/smiles/3.png\'>',
        ':('=>'<img src=\'http://studentosi.ru/uploads/smiles/10.png\'>',
        'xD'=>'<img src=\'http://studentosi.ru/uploads/smiles/23.png\'>',
        ':D'=>'<img src=\'http://studentosi.ru/uploads/smiles/1.png\'>',
        ':P'=>'<img src=\'http://studentosi.ru/uploads/smiles/34.png\'>',
        ':*'=>'<img src=\'http://studentosi.ru/uploads/smiles/24.png\'>'
    ));
    return $modified;
}
...
//Тип кэша
$memcache = false;
if ($config['cache_type'] == 'memcache') {
    $memcache_key = $config['memcache_key'];
	if (function_exists('memcache_connect')) {
		$memcache_server = explode(":", $config['memcache_server']);
		$memcache = @memcache_connect($memcache_server[0], $memcache_server[1]);
		if($memcache AND function_exists('memcache_set_compress_threshold')){
			memcache_set_compress_threshold($memcache, 20000, 0.2);
		}
	}
}
//Записываем системный кэш
function creat_system_cache($prefix, $cache_text){
    global $memcache, $memcache_key;
	if($memcache){ 
	    $key = md5($prefix);
		memcache_set($memcache, $memcache_key.$key, $cache_text, MEMCACHE_COMPRESSED, 0);
	} else {
...
//Читаем системный кэш
function system_cache($prefix) {
    global $memcache, $memcache_key;
	if($memcache) {
	    $key = md5($prefix);
        return memcache_get($memcache, $memcache_key.$key);
	} else {
		$filename = ENGINE_DIR.'/cache/system/'.$prefix.'.php';
...
function mozg_clear_cache_file($prefix) {
	...
	if($memcache){
	   $key = md5($prefix);
	   $memcache->delete($memcache_key.$key);
	} else
	@unlink(ENGINE_DIR.'/cache/'.$prefix.'.tmp');
}
//Массовое удаление кэша
...
function megaDate($date, $func = false, $full = false){
	...
	$date_time = time() - $date;
	if ($date_time < 1) 
        return $tpl->set('{date}', 'только что');
    elseif ($date_time < 60) 
        return $tpl->set('{date}',	$date_time . declOfNum($date_time, array(' секунду назад', ' секунды назад ', ' секунд назад')));		
	elseif ($date_time < 3600) 
	    return $tpl->set('{date}', round($date_time / 60) . declOfNum(round($date_time / 60), array(' минуту назад', ' минуты назад ', ' минут назад')));
	elseif ($date_time < 86400) 
	    return $tpl->set('{date}', round($date_time / 3600) . declOfNum(round($date_time / 3600), array(' час назад', ' часа назад ', ' часов назад')));
	elseif(date('Y-m-d', $date_comm) == date('Y-m-d', $server_time))
...
function megaDateNoTpl($date, $func = false, $full = false){
	...
	elseif ($date_time < 86400) 
	    return $tpl->set('{date}', round($date_time / 3600) . declOfNum(round($date_time / 3600), array(' час назад', ' часа назад ', ' часов назад')));
	else*/if(date('Y-m-d', $date) == date('Y-m-d', $server_time))
...
function AntiSpam($act, $text = false){
	...
	*/
	...
	$max_msg = 30; #макс. сообщений не друзьям
	$max_wall = 25; #макс. записей на стену
	$max_identical = 50; #макс. одинаковых текстовых данных
	...
	elseif($act == 'wall'){
		...
		if($check['cnt'] >= $max_wall){
			...
			die('У вас лимит. В день можно писать 20 постов.');
