...
if($logged){
	...
	switch($act){
		...
		case "save":
			...
			$check = $db->super_query("SELECT time FROM `".PREFIX."_notes` WHERE owner_user_id = '{$user_id}' order by id DESC limit 0,1");
            if($check['time'] > (time()-60)) die('ddos'); //60 - это минимальное время через которое можно добавить заметку.
			//Подключаем парсер
			...
			if(strlen($title) > 0 && strlen($text) > 0){
				$db->query("INSERT INTO `".PREFIX."_notes` SET owner_user_id = '{$user_id}', title = '{$title}', full_text = '{$text}', date = NOW(),time='{$server_time}'");
				$db_id = $db->insert_id();
