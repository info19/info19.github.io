				...
				foreach($attach_arr as $attach_file){
					...
					//Видео
						...
						if($cnt_attach_video == 1 AND preg_match('/(photo|photo_u)/i', $row_wall['attach']) == false){
							...
							$row_video['video'] = strtr($row_video['video'], array('width="770"' => 'width="175"', 'height="420"' => 'height="131"'));
...
<div class="wall_tell_info"><div class="wall_tell_ava"><a href="/{$tell_link}{$row_wall['tell_uid']}" onClick="Page.Go(this.href); return false"><img src="{$avaTell}" width="30" /></a></div><div class="wall_tell_name"><a href="/{$tell_link}{$row_wall['tell_uid']}" onClick="Page.Go(this.href); return false">{$rowUserTell['user_search_pref']}</a></div><div class="wall_tell_date">{$dateTell}</div></div>{$row_wall['text']}
<div class="clear"></div>
</div>
HTML;
			}
			...
			$rowxxd = $db->super_query("SELECT user_photo, user_search_pref, user_status, short_link FROM `".PREFIX."_users` WHERE user_id = '{$row_wall['uid']}'");
                
                if($rowxxd['short_link']) $alias_name = $rowxxd['short_link'];
                else $alias_name = $row_wall['uid'];
                
                if($row_wall['view_author']==1 and $row_wall['uid']!=0) {
					if ($rowxxd['user_photo']) {
						
						$tpl->set('{ava_two}', '/uploads/users/'.$row_wall['uid'].'/100_'.$rowxxd['user_photo']);
					} else {
						
						$tpl->set('{ava_two}', '{theme}/images/100_no_ava.png');
					}
				
				$tpl->set('{author_view}', $rowxxd['user_search_pref']);
				$tpl->set('{auth_id}', $row_wall['uid']);
				$tpl->set('{user_status}', $rowxxd['user_status']);
				}
